...
// optionalAuth (not public): when a token is present it binds the request to the
// LOGGED-IN user's own institute, so an institute admin reads THEIR settings —
// the same doc PUT /settings writes to. Without this, an admin on the shared
// apex domain read the DEFAULT/platform settings while their saves went to their
// own institute, so flags like onboardingCompleted never appeared to "stick".
// Unauthenticated visitors still resolve by hostname (public branding), unchanged.
