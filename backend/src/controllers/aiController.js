...
- "keyPoints": array of 3 to 5 SHORT strings — the crisp, exam-ready takeaways a student should remember about this question's concept (each a compact phrase or one short sentence, NOT a full paragraph). Cover the decisive fact behind the correct answer plus the closely-related facts that distinguish the distractors (e.g. what each other item actually is). No leading bullet characters or numbering, no markdown. Wrap any math/number in $...$; never use "$" for money.
- "quickRecall": a SINGLE short memory hook / mnemonic for instant recall — one line, ideally under ~12 words (e.g. "Malaria = female Anopheles"). Keep it punchy and specific to this question. No markdown; wrap math in $...$.
...
COLUMNS / STATEMENTS ARE ARRAYS OF SEPARATE ITEMS (CRITICAL): put EACH item as its OWN element of the "columnA" / "columnB" array. NEVER pack multiple items into a single element, and NEVER embed list markers such as "II.", "III.", "IV." (or "2.", "3.", "4.") inside one element — doing so collapses the ENTIRE column into a single row and only the first item shows (the rest are lost). Example of CORRECT columnB: ["Preserves records of a business.","Keeps a parent organisation's records.","Holds a nation's public records.","Builds a subject-specific repository."] — four SEPARATE strings. WRONG: ["Preserves records of a business. II. Keeps a parent org's records. III. Holds public records. IV. Builds a repository."] (one blob). "columnA" and "columnB" MUST contain the SAME number of separate elements, and every element must be non-empty. Do NOT prefix columnA / columnB / statement items with numbers or roman numerals (no "1.", "I.") — the app numbers Column A (1,2,3,4), Column B (I,II,III,IV) and statements (1,2,3,4) automatically.
...
FLASHCARD EXTRAS ARE MANDATORY FOR EVERY QUESTION (do NOT skip): besides "explanation", EVERY question object — of EVERY type (mcq, numericalmcq, matching, statement, pair, pairselect, assertion, table, journal, ledger, rearrange, diagram) and EVERY difficulty — MUST include BOTH a non-empty "keyPoints" AND a non-empty "quickRecall". "keyPoints" is a JSON array of 3 to 5 SHORT exam-ready takeaways (the decisive fact behind the correct answer PLUS the facts that distinguish the other options); "quickRecall" is ONE short memory hook / mnemonic for instant recall (one line, ideally under ~12 words, e.g. "Malaria = female Anopheles"). These two fields power the answer FLASHCARD shown to students, so NEVER omit them, NEVER leave them blank, and NEVER fill them for only some questions — a question missing "keyPoints" or "quickRecall" is INCOMPLETE. No markdown, no leading bullet/number characters; wrap any math/number in $...$ and never use "$" for money. Before finalising EACH question, CONFIRM it has "keyPoints" (3–5 filled items) and a filled "quickRecall".
  ...
  return (Array.isArray(list) ? list : [])
    .map((q) => {
      ...
      const out = {
        ...
        // Flashcard extras (shown on the answer flashcard). keyPoints = 3–5 crisp
        // takeaways; quickRecall = a short mnemonic / memory hook.
        keyPoints: arrStr(q?.keyPoints).map((s) => s.trim()).filter(Boolean).slice(0, 6),
        quickRecall: asStr(q?.quickRecall).trim(),
        status: "published",
