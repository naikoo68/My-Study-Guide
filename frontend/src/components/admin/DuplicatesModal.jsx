...
/**
 * "Find duplicates" — scans the EXISTING subjects (under a stream) or topics
 * (under a subject) and groups the ones that are duplicates or overlaps (not
 * just identical names — synonyms and broader/combined wordings too), so the
 ...
 *  - items: array of the current items — each has _id and name|title
 ...
 *  - onClose: () => void
 */
export default function DuplicatesModal({ level, parentName, fetchGroups, onDelete, bulkProgress, onClose }) {
                  ...
                  <div className="space-y-0.5">
                    {g.duplicates.map((d) => (
                      <label key={d.id} className="flex cursor-pointer items-center gap-2 rounded px-1 py-1 text-sm hover:bg-slate-100 dark:hover:bg-slate-700">
                        ...
                        <span className={toDelete.has(d.id) ? "line-through text-slate-400" : ""}>{d.name}</span>
                      </label>
                    ))}
                  </div>
                </div>
