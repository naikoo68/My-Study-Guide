  ...
  const runGenerate = async (append = false) => {
    ...
    try {
      const { jobId, requested } = await aiService.generate({
        ...
        subtopics: subtopics.trim() || undefined, // spread the questions across these
        plan,
        notes: notes.trim() || undefined,
      ...
      <div className="min-h-full w-full max-w-none animate-scale-in card m-0 rounded-none p-4 sm:rounded-2xl sm:p-6">
        ...
        {status && !status.enabled ? (
          ...
          <>
            ...
            {/* Optional: subtopics to spread across + include numerical questions. */}
            {task === "generate" && (
              <div className="mt-4 space-y-3">
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-400">
                    The questions will be spread across these subtopics, drawing the facts from your source material above.
                  </p>
                </div>
